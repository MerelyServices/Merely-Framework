...
class Obsolete(commands.Cog):
	...
	async def music(self, ctx):
		if globals.musicbuddy:
			await ctx.message.channel.send(f"to use {globals.name} for music, please add the {globals.name} music bot!\nhttps://discordapp.com/oauth2/authorize?client_id={globals.musicbuddy}&scope=bot&permissions=0")
	@commands.command(pass_context=True, no_pm=False)
	async def nsfw(self, ctx):
		await ctx.message.channel.send(f"{globals.name} supports nsfw through using the `{globals.prefix_short}images` command in an nsfw channel.")