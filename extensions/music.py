...
class Music(commands.Cog):
  ...
  async def music(self, ctx:commands.Context):
    if ctx.guild.get_member(int(self.bot.config['music']['music_companion'])) is None:
      await ctx.reply(self.bot.babel(ctx, 'music', 'promo', invite=f"https://discord.com/oauth2/authorize?client_id={self.bot.config['music']['music_companion']}&scope=bot&permissions=0"))
