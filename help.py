...
dhelp={
	...
	'meme':"***merely meme [add (url) | #(number) | delet]***\nsends a random meme to the channel. add followed by a link to an image adds a meme, delet removes the previously posted one.\n\n*merely meme is designed to avoid repeating memes for as long as possible*",
	...
	async def hint(self,ctx):
		...
		await emformat.make_embed(ctx.message.channel,'',title="Did you know...",
				...
				["`merely help` followed by a command name explains the command in more detail...",
				 "`merely meme delet` moves memes on to a list for the bot mods to review.",
				 "click the title of any command's message to be taken to the online docs. <:soon:233642257817927680>",
				 ...
				 "`merely about` shows off what merely has to offer to your server and provides an invite link.",
				 "set the playing status of merely with `merely playing <status>.`",
				 "some commands are restricted to the creator, trusted moderators and server owners, like `blacklist` and `die`",
